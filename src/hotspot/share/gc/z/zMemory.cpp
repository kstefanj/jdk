...
zoffset ZMemoryManager::alloc(size_t size) {
...
zoffset ZMemoryManager::alloc_at_most(size_t size, size_t* allocated) {
  ZLocker<ZLock> locker(&_lock);
...
ZMemory* ZMemoryManager::remove_first() {
  ZLocker<ZLock> locker(&_lock);
  return _freelist.remove_first();
}
