...
inline void G1FullGCMarker::scan_array_chunk(objArrayOop obj, size_t start, size_t end) {
...
inline void G1FullGCMarker::dispatch_task(const ScannerTask& task, bool stolen) {
  if (task.is_partial_array_state()) {
    ...
    scan_partial_array(task.to_partial_array_state(), stolen);
  } else {
    oop obj = task.to_oop();
    ...
    if (obj->is_objArray()) {
      ...
      start_partial_array_scanning((objArrayOop)obj);
    } else {
