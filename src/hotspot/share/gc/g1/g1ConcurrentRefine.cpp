...
void G1ConcurrentRefineThreadControl::update_threads_cpu_time() {
  ...
  CollectorCPUTimeCounters* counters = G1CollectedHeap::heap()->cpu_time_counters();
  ThreadTotalCPUTimeClosure tttc(counters->get_counter(CollectorCPUTimeGroups::gc_conc_refine), counters);
