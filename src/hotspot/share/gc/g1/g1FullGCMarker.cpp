...
void G1FullGCMarker::scan_partial_array(PartialArrayState* state, bool stolen) {
  ...
  scan_array_chunk(obj_array, claim._start, claim._end);
}

void G1FullGCMarker::start_partial_array_scanning(objArrayOop obj) {
  ...
  if (array_length > 0) {
    ...
    scan_array_chunk(obj, 0, initial_chunk_size);
  }
}
